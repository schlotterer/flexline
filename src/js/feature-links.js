...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function toggleButtonPosition(buttonToPosition) {
    ...
    if (isScrolled) {
      const isSmallScreen = window.matchMedia('(max-width: 781.98px)').matches;
  ...
  function centerButtonInHeader(buttonToCenter) {
    ...
    if (headerContainer) {
      ...
      const offset =
        (headerContainer.offsetHeight - buttonToCenter.offsetHeight) / 2;
  ...
  function addCloseButton(menuContainer) {
    ...
    if (!closeButton) {
      ...
      closeButton.innerHTML =
        '<span class="material-symbols-outlined">' + iconClose + '</span>';
